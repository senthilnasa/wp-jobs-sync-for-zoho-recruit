...
function Edit( { attributes, setAttributes } ) {
	...
	return (
		<div { ...blockProps }>
			<InspectorControls>
				<PanelBody
					title={ __( 'Listing', 'jobs-sync-for-zoho-recruit' ) }
				>
					<RangeControl
						label={ __(
							'Jobs to show',
					...
					<SelectControl
						...
						onChange={ ( listingStyle ) =>
							setAttributes( { listingStyle } )
						}
					/>
				...
				>
					...
					<TextControl
						...
						help={ __(
							'Term slug or name. Separate several with commas.',
							'jobs-sync-for-zoho-recruit'
						) }
						value={ attributes.department }
						onChange={ ( department ) =>
							setAttributes( { department } )
						}
					/>
					<TextControl
...
registerBlockType( metadata.name, {
	...
	save() {
		...
		// the plugin's PHP renderer, so there is no markup to save.
		return null;
	},
} );
